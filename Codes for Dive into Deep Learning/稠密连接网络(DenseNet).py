...
def transition_block(in_channels,out_channels):
    blk=nn.Sequential(nn.BatchNorm2d(in_channels),nn.ReLU(),nn.Conv2d(in_channels,out_channels,kernel_size=1),nn.AvgPool2d(kernel_size=2,stride=2))
    return blk
...
for i,num_convs in enumerate(num_convs_in_dense_blocks):
    ...
    if i!=len(num_convs_in_dense_blocks)-1:
        net.add_module('transition_block %d'%i,transition_block(num_channels,num_channels//2))
